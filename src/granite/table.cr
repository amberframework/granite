...
module Granite::Tables
  ...
  macro table(name)
    @[Granite::Table(name: {{(name.is_a?(SymbolLiteral) ? name.id.stringify : (name.is_a?(StringLiteral) ? name : name.stringify)) || nil}})]
    class ::{{@type.name.id}}; end
  end

  # specify the database connection you will be using for this model.
  macro connection(name)
    class_getter adapter : Granite::Adapter::Base = Granite::Connections[{{(name.is_a?(SymbolLiteral) ? name.id.stringify : (name.is_a?(StringLiteral) ? name : name.stringify))}}] || raise "No registered connection with the name '{{name.id}}'"
  end
end
